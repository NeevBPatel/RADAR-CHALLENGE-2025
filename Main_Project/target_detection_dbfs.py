'''
   ...
   Original code from Marshall Bruner, Colorado State University and Jon Kraft, Analog Devices Inc.
   https://github.com/brunerm99/ADI_Radar_DSP
   Modified by Neev Patel to use data received circularly
'''
'''
import numpy as np
...
def cfar(X_k, num_guard_cells, num_ref_cells, bias=1, cfar_method='average',
    ...
    N = X_k.size
    cfar_values = np.ma.masked_all(X_k.shape)
    for center_index in range(0, N):
        x = num_guard_cells + num_ref_cells
        
        diff = center_index - x
        negative = abs(diff) - center_index
        positive = center_index
        
        if diff < 0:
            pos = X_k[0:positive]
            neg = X_k[-1*negative:0]
            lower_nearby = np.concatenate((neg,pos))
            
        else:
            min_index = center_index - x
            min_guard = center_index - num_guard_cells
            lower_nearby = X_k[min_index:min_guard]
        
        addition = center_index + x
        positive = center_index
        negative = addition - positive
        
        if addition > N:
            pos = X_k[positive:]
            neg = X_k[0:negative]
            upper_nearby = np.concatenate((pos,neg))
            
        else:
            max_index = center_index + (x) + 1
            max_guard = center_index + num_guard_cells + 1
            upper_nearby = X_k[max_guard:max_index]
